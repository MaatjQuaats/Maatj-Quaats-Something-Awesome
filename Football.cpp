...
const int CLUBS = 4;
const int PLAYERS_PER_CLUB = 2;
const int MATCHES = (CLUBS * (CLUBS - 1)) / 2;
...
private:
    ...
    int home[MATCHES];
    int away[MATCHES];
    int m_home_goals[MATCHES];
    int m_away_goals[MATCHES];
    bool played_flag[MATCHES];
    int fixture_count;
...
public:
    championship()
    {
        ...
        for (int i = 0; i < MATCHES; i++)
        {
            ...
            this->played_flag[i] = 0;
        }
    }
    int points(int i, const int won[], const int drawn[])
    {
        return won[i] * 3 + drawn[i];
    }
    int gd(int i, const int gf[], const int ga[])
    {
        return gf[i] - ga[i];
    }
    void sort_table(int order[CLUBS])
    {
        for (int i = 0; i < CLUBS; i++)
            order[i] = i;
        ...
        for (int i = 0; i < CLUBS - 1; i++)
        {
            ...
            {
                int L = order[j], R = order[j + 1];
                bool swap_needed = false;

                int pL = points(L, won, drawn), pR = points(R, won, drawn);
                if (pL < pR)
                    swap_needed = true;
                else if (pL == pR)
                {
                    int gdL = gd(L, gf, ga), gdR = gd(R, gf, ga);
                    if (gdL < gdR)
                        swap_needed = true;
                    else if (gdL == gdR)
                    {
    ...
    int input_goals_for_club(int ci)
    {
    ...
    void user_interface()
    {
        ...
        while (!quit_requested())
        {
            ...
            if (current_window == 0)
                draw_text("Mini Football League — Menu", color_white(), 16, 20);
            ...
            else if (current_window == 2)
            {
                ...
                for (int i = 0; i < fixture_count; i++)
                {
                    ...
                    if (played_flag[i])
                        line = std::to_string(i + 1) + ". " + home_name + " " + std::to_string(m_home_goals[i]) + " - " + std::to_string(m_away_goals[i]) + " " + away_name;
                    else
            ...
            else if (current_window == 4)
            {
                ...
                if (mouse_clicked(LEFT_BUTTON) &&
                    point_in_rectangle(mouse_x(), mouse_y(), 0, 540, 540, 60))
                {
                    current_window = 0;
                }
            }
...
private:
    ...
    int find_user(const string &u) const
    {
